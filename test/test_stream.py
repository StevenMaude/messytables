...
class StreamInputTest(unittest.TestCase):
    ...
    def test_http_csv(self):
        ...
        httpretty.register_uri(httpretty.GET, url, body=horror_fobj("long.csv").read(), content_type="application/csv")
        fh = urlopen(url)
    ...
    def test_http_csv_encoding(self):
        ...
        httpretty.register_uri(
            httpretty.GET, url, body=horror_fobj("utf-16le_encoded.csv").read(), content_type="application/csv"
        )
        fh = urlopen(url)
        table_set = CSVTableSet(fh)
        row_set = table_set.tables[0]
    ...
    def test_http_xls(self):
        ...
        httpretty.register_uri(
            httpretty.GET, url, body=horror_fobj("simple.xls").read(), content_type="application/ms-excel"
        )
        fh = urlopen(url)
