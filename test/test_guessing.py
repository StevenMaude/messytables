...
class TypeGuessTest(unittest.TestCase):
    ...
    def test_strict_guessing_handles_padding(self):
        ...
        assert guessed_types == [StringType(), StringType(), DecimalType()]
    ...
    def test_non_strict_guessing_handles_padding(self):
        ...
        assert len(guessed_types) == 3
        assert guessed_types == [IntegerType(), StringType(), DecimalType()]
    ...
    def test_guessing_uses_first_in_case_of_tie(self):
        csv_file = io.BytesIO(b"""
            ...
            1500""")
        rows = CSVTableSet(csv_file).tables[0]
        guessed_types = type_guess(rows.sample, types=[DecimalType, IntegerType], strict=False)
        assert guessed_types == [DecimalType()]

        guessed_types = type_guess(rows.sample, types=[IntegerType, DecimalType], strict=False)
        assert guessed_types == [IntegerType()]
    ...
    def test_integer_and_float_detection(self):
        ...
        assert helper("123") is True
        assert helper(123.0) is True
        assert helper("123.0") is True
        assert helper(123.1) is False
        assert helper("123.1") is False
