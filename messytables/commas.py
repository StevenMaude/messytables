...
import messytables
from messytables.compat23 import byte_string, unicode_string
...
class CSVRowSet(RowSet):
    ...
    def _dialect(self):
        ...
        try:
            dialect = csv.Sniffer().sniff(sample, delimiters=["\t", ",", ";", "|"])  # NATIVE
    ...
    def raw(self, sample=False):
        ...
        try:
            for row in csv.reader(rows(), dialect=self._dialect, **self._overrides):
                yield [Cell(to_unicode_or_bust(c)) for c in row]
        except csv.Error as err:
            if "newline inside string" in unicode_string(err) and sample:
                pass
            elif "line contains NULL byte" in unicode_string(err):
                pass
            else:
